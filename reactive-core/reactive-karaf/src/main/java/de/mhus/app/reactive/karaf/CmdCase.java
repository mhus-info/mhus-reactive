...
public class CmdCase extends AbstractCmd {
    ...
    public Object execute2() throws Exception {
        ...
        if (cmd.equals("updatefull")) {
            try (PCaseLock lock =
                    EngineUtil.getCaseLock(api.getEngine(), parameters[0], "cmdcase.updatefull")) {
            ...
            try (PCaseLock lock =
                    EngineUtil.getCaseLock(api.getEngine(), parameters[0], "cmdcase.setoption")) {
                PCase caze = lock.getCase();
            ...
            try (PCaseLock lock =
                    EngineUtil.getCaseLock(api.getEngine(), parameters[0], "cmdcase.setparam")) {
                PCase caze = lock.getCase();
        ...
        if (cmd.equals("resave")) {
            ...
            for (String id : parameters) {
                try (PCaseLock lock =
                        EngineUtil.getCaseLock(api.getEngine(), parameters[0], "cmdcase.cancel")) {
                    PCase caze = lock.getCase();
