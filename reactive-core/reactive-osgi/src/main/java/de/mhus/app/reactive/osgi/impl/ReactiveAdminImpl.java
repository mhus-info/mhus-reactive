...
import de.mhus.app.reactive.engine.util.LogConfiguration;
import de.mhus.app.reactive.engine.util.PoolValidator;
import de.mhus.app.reactive.engine.util.PoolValidator.Finding;
import de.mhus.app.reactive.engine.util.PoolValidator.LEVEL;
...
public class ReactiveAdminImpl extends MLog implements ReactiveAdmin {
    ...
    public void undeploy(String name) throws MException {
        ...
        if (info == null) throw new MException(RC.NOT_FOUND, "Process {1} not found", name);
        if (info.deployedName == null)
            throw new MException(RC.BUSY, "Process {!} is not deployed", name);
    ...
    protected void updateStorageDataSource() throws InvalidSyntaxException, MException {
        ...
        if (refs.size() == 0)
            throw new MException(RC.BUSY, "datasource {1} not found", storageDsName);
    ...
    protected void updateArchiveDataSource() throws InvalidSyntaxException, MException {
        ...
        if (refs.size() == 0)
            throw new MException(RC.BUSY, "datasource {1} not found", archiveDsName);
