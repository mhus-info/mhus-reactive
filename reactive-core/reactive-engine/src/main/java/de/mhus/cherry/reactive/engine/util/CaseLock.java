...
public class CaseLock extends MLog implements Closeable {
    ...
    public static Scope spanStart(PCase caze, String operation, Object ... tagPairs ) {
		...
		Scope scope = null;
		if (ctx == null) 
			scope = ITracer.get().tracer().buildSpan(operation).startActive(true);
		else {
			scope = ITracer.get().tracer().buildSpan(operation).addReference(References.FOLLOWS_FROM, ctx).startActive(true);
		}
		return scope;
	}
