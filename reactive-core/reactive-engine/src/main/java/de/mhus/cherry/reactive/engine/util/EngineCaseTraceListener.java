...
public class EngineCaseTraceListener implements EngineListener {
    ...
    private synchronized Log getLog(UUID id) {
        Log log = logs.get(id);
        if (log == null) {
            File file = MApi.getFile(MApi.SCOPE.LOG, "reactive_" + id + ".log");
    ...
    @Override
    public void startCase(MUri originalUri, MUri uri, IProperties properties, EProcess process, EPool pool,
    ...
    @Override
    public void createActivity(RuntimeNode runtimeNode, PNode flow, PCase pCase, PNode previous, EElement start) {
        Log log = getLog(pCase);
        log.i("createStartNode",runtimeNode,flow,pCase,previous,start);
    }

    @Override
    public void executeStart(RuntimeNode runtime, PNode flow, EElement start, AActivity<?> activity) {
        Log log = getLog(flow);
