...
public class ExchangeUtil {
    ...
    public static INode caseToINode(PCase caze) {
        INode nc = new MNode();
        ...
        for (Entry<String, Object> entry : caze.getOptions().entrySet()) {
            ...
            no.setString("_class_" + entry.getKey(), entry.getValue().getClass().getCanonicalName());
        }
        ...
        for (Entry<String, Object> entry : caze.getParameters().entrySet()) {
            ...
            np.setString("_class_" + entry.getKey(), entry.getValue().getClass().getCanonicalName());
        }
        return nc;
    }

    public static PCase nodeToPCase(INode in) throws MException {
        PCase out = new PCase();
        ...
        if (in.isProperty("closeActivity"))
            setValue(out, "closeActivity", UUID.fromString(in.getString("closeActivity")));
    ...
    public static INode nodeToINode(PNode node) {
        INode nn = new MNode();
    ...
    public static PNode nodeToPNode(INode in) throws MException {
        PNode out = new PNode();
        ...
        if (MString.isSet(in.getString("suspendedState", "")) && !"null".equals(in.getString("suspendedState", "")))
            out.setSuspendedState(STATE_NODE.valueOf(in.getString("suspendedState")));
        if (MString.isSet(in.getString("startState", "")) && !"null".equals(in.getString("startState", "")))
