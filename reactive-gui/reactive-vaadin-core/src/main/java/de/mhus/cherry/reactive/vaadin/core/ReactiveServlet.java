...
@Component(provide = Servlet.class, properties = { "alias=/reactive" }, name="ReactiveGUI",servicefactory=true)
@VaadinServletConfiguration(ui=ReactiveUi.class, productionMode=true)
public class ReactiveServlet extends VaadinServlet {
			...
			try {
				...
				if (vaadinSession != null) {
		    		for (UI ui : vaadinSession.getUIs()) {
		    			if (ui instanceof ReactiveUi)
		    				((ReactiveUi)ui).requestEnd();
		    		}
				}
    		} catch (Throwable t) {
